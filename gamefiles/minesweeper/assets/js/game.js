...
class mineSweeper {
  ...
  init() {
    ...
    if (this.number_of_cells > 2500) {
      alert('Too big to handle, please have less than 2500 cells.');
      return false;
    } // Prevent for big Matrix
    if (this.number_of_cells <= this.number_of_bombs) {
      alert('more bombs than cells not allowed!');
      return false;
    } // Validation w.r.t number of cells for bombs
    ...
    grid_data.forEach(function(isBomb, i) {
      ...
      if (!isBomb) {
        var neighbors = neighbors_cords.map((xy) => {
          return grid_data[getIndex(xy[0], xy[1])];
        });
        mine.mine_count = neighbors.filter((neighbor_bomb) => {
          return neighbor_bomb;
        }).length;
      }
      mine.classList.add('x' + x, 'y' + y);
      mine.neighbors = neighbors_cords.map((xy) => {
